...
import { 
  ...
  deleteAgent, updateAgent, 
  deleteInscription, updateInscription,
  deleteEstudianteActivo, updateEstudianteActivo
} from '@/utils/api'
import EditDialog from './edit-dialog'
...
export default function ActionButtons({ item, onDelete, onUpdate, itemType, schema, idField }) {
  ...
  const getDeleteFunction = () => {
    switch (itemType) {
      ...
      case 'agente': return deleteAgent
      case 'inscripcion': return deleteInscription
      case 'estudiante_activo': return deleteEstudianteActivo
      default: return null
    }
  }
  ...
  const getUpdateFunction = () => {
    switch (itemType) {
      ...
      case 'agente': return updateAgent
      case 'inscripcion': return updateInscription
      case 'estudiante_activo': return updateEstudianteActivo
      default: return null
    }
  }
  ...
  const handleDelete = async () => {
    ...
    if (confirmationId !== item[idField].toString()) {
      toast({
        title: 'Error',
        description: 'La cédula ingresada no coincide.',
        variant: 'destructive',
      })
      return
    }
    ...
    if (!deleteFunction) {
      toast({
        title: 'Error',
        description: `Tipo de elemento no soportado: ${itemType}`,
        variant: 'destructive',
      })
      return
    }
      ...
      <Dialog open={isDeleteDialogOpen} onOpenChange={setIsDeleteDialogOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Confirmar eliminación</DialogTitle>
            <DialogDescription>
              Esta acción no se puede deshacer. Por favor, confirme la cédula del estudiante activo.
            </DialogDescription>
          </DialogHeader>
          <div className="grid gap-4 py-4">
            <p className="text-red-500 font-medium">Cédula: {item[idField]}</p>
            <Input
