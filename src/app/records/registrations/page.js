...
// const SCHEMA = [
//   { name: 'inscripcion_id', label: 'ID', type: 'text', hidden: true },
...
//   const columns = SCHEMA.filter(field => !field.hidden).map(field => ({
//     key: field.name,
//     label: field.label,
//     sortable: true,
...
export default function Inscripciones() {
  ...
  const totalInscriptions = inscriptions.length
  const averageStudentsPerProgram = programs.length > 0 ? (totalInscriptions / programs.length).toFixed(2) : 0
        ...
        {showSummary && (
          <div className="grid gap-4 md:grid-cols-2 mb-6">
            ...
            <Card>
              ...
              <CardContent>
                <div className="text-2xl font-bold">{averageStudentsPerProgram}</div>
